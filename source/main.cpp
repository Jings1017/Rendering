...
static int add_obj(unsigned int program, const char *filename, const char *texbmp)
{
	...
	//glBufferData(...);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, shapes[0].mesh.indices.size()*sizeof(float), shapes[0].mesh.indices.data(), GL_STATIC_DRAW);
...
static void render()
{
	...
	proj_matrix = glm::perspective(glm::radians(90.0f), (float)4.0 / (float)3.0, 0.1f, 100.0f);
